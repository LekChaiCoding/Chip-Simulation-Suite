"""JTWPA-specific fitting tools — the vendored JosephsonCircuit surface.

Separated out on 2026-08-04. These wrap the vendored reference scripts for one
device family (a travelling-wave parametric amplifier) and are meaningless for
anything else: `run_abcd_fit` reads a `stub_length_sweep.dat` produced by the
JTWPA stub sweep, and the two Julia entry points call
`JosephsonCircuits.jl` scripts from the vendored `JosephsonCircuit/` tree.

Everything device-agnostic stays in `..fitting` — notably `run_generic_fit`,
which runs whatever fitting script the caller names and has nothing to do with
this device. Keeping the two in one module made "is this tool general?" a
question you had to answer by reading the body.

The vendored trees these read are ABSENT from this checkout, so every function
here fails with "not found" before touching anything; `describe_config` reports
which. That absence is load-bearing for the gating decision recorded in
`test_tool_gate_surface.py`, and a tripwire there fails if a tree is restored.
"""
...
# `_collect` is shared with the generic module rather than duplicated: it only
# globs an output directory and is not specific to this device.
from ..fitting import _collect  # noqa: F401
